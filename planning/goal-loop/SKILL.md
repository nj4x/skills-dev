...
- **One objective, one stop condition.** A contract is not a backlog.
...
- **4,000-char hard limit on the entire goal contract.** The harness rejects longer input with `Goal condition is limited to 4000 characters (got N)` — the count covers the *whole* contract body (objective + read-first + constraints + validate + stop-when + everything else), not just the objective line. If a draft exceeds 4,000 chars, compress it before handing it over:
  - Move long detail (seed defect lists, file inventories, step-by-step validation recipes) into a file (`GOAL_BRIEF.md`/`PLAN.md`) committed in the repo, and make the contract point to it: `Read first: GOAL_BRIEF.md` — keep the contract itself a compact pointer.
  - Cut redundancy: collapse repeated phrasing, drop parenthetical asides, delete restated constraints, and prefer terse literal lists over prose. Preserve every distinct instruction, stop condition, and constraint — compression must not drop requirements.
  - After trimming, verify the character count (e.g. `wc -c`) is under 4,000 before delivering. State the final count to the user.
...
| Contract is active but auto-continuation won't trigger | Stuck in Plan mode — plan-only work doesn't satisfy the contract loop. Draft the plan, then switch to Goal execution. |
