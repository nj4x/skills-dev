...
- **`reason: "worker_offline"`** — a watchdog is running and reports no live worker, caught before enqueuing so it returns instantly. A restart is already due within about five minutes: wait and retry once. This reason never appears when no watchdog is running — the pool is then assumed live, and a pool that is in fact down shows up as `timeout` after the full block instead (ADR-0079). So a `timeout` on the first question of a session is worth reading as "ask the human whether a worker is up", not as "the worker is thinking".
