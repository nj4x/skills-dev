...
| `--no-build` | flag (present/absent) | absent | Skip Step 2 entirely: set `BUILD_STATUS = WAIVED`, `OPENAPI_STATUS = WAIVED`, `REVIEW_MODE = PARTIAL`, and proceed to Step 3 without running any build command. The flag *is* the risk acceptance — no prompt and no `skip with risk accepted` text needed. Wins over `--build-cmd` when both are supplied (ADR-0074). |
| `--build-cmd` | `"<command>"` | project-type default | Override the command detected in Step 2.1, and satisfy the Unknown project type — a supplied command leaves nothing to guess, so Step 2.2 announces it rather than asking. Gradle compliance (`openapi3` required) applies to supplied commands too; a non-compliant one stops the review instead of running (ADR-0075). |
...
| `/code-review --no-build` | Skip the build gate entirely; `BUILD_STATUS = WAIVED`, review runs in PARTIAL mode. |
| `/code-review --build-cmd "./gradlew build openapi3"` | Run the supplied command instead of the detected default; Gradle `openapi3` compliance still enforced. |
...
7.5. **Lineage enforcement (ADR-0061).** After the standard review passes complete, run these two additional checks (workflow.md Step 10.5). Append findings in a **Lineage** subsection of the report (step 8), separate from other findings.
   ...
   **Primary — Code-to-spec alignment (grade-impacting):**
   - Retrieve the ticket's `**Spec**:` slug (spec-linked) or `**Source ADR**:` path (adr-direct) from the diff context or PR description. Branch on anchor type:
     - **Spec-linked** (`**Spec**:` slug): read `.scratch/<slug>/spec.md`. If the file does not exist or has no frontmatter, skip this check — Group F (Critic) will catch the missing spec. If resolved, compare against the spec's acceptance criteria.
     - **ADR-direct** (`**Source ADR**:` path, no `**Spec**:` slug): compare code changes against the ADR's Decision and Consequences sections.
   - Apply the same two-way comparison in both branches:
     - Code adds behavior not described → **Major** finding — "Undocumented scope creep: `<behavior>` not in spec."
     - Code omits specified behavior → **Major** finding — "Incomplete implementation: `<requirement>` specified in spec but not present in code."
...
- **Announce-and-run** the build: print the resolved command in your response text at the **announced** consent level, then execute it (ADR-0073). The build gate's one remaining ask is the Unknown project type with no `--build-cmd`, where no command can be resolved.
