...
- **A threaded poll must fall back to top-level `pending/`/`claimed/`, not `locate()` alone.** Amended during implementation: this ADR's resolution rule assumed a threaded record always sits under `queue/threads/<id>/`, but per ADR-0073 a thread's *first* message stays in top-level `pending/` until a worker claims it. A strict `locate()` lookup would therefore report `unknown_handle` for a legitimately pending first message — terminal, on the very first poll of every new thread. `read_record()` searches the thread base, then top-level, admitting a hit only when the record's own `thread_id` equals the one polled with — so an unthreaded handle polled under a `thread_id` (and the reverse) still reports `unknown_handle`. This mirrors the fallback `read_first_in_thread()` already makes for the same reason.

  A side effect: the misrouted follow-up described under *Thread invariant* is now pollable rather than reported as `unknown_handle`, since it carries the same `thread_id` in top-level `pending/`. That section's warning still stands on its real point — the follow-up is claimable by a worker outside the thread's session, so it is answered without the thread's context — but the caller can at least observe it.
